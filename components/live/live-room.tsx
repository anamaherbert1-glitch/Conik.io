...
export default function LiveRoom({ tokenUrl, tokenBody, host = false }: Props) {
  ...
  useEffect(() => {
    ...
    room.on(RoomEvent.LocalTrackPublished, publication => {
      if (publication.track && host) {
        if (publication.track.kind === Track.Kind.Audio) return
        attachVideo(publication.track, getContainer(publication.source, true))
      }
    })
  ...
  async function startOrChangeScreenShare() {
    ...
    try {
      ...
      setScreenShare(false); setMediaMessage(mediaErrorMessage(error, 'screen'))
    } finally { setScreenShareChanging(false) }
  }
  ...
  async function stopScreenShare() {
    ...
    if (!room || !host || screenShareChanging) return
    setScreenShareChanging(true)
    try { await room.localParticipant.setScreenShareEnabled(false); setScreenShare(false); setMediaMessage('') } catch (error) { setMediaMessage(mediaErrorMessage(error, 'screen')) } finally { setScreenShareChanging(false) }
  }
  ...
  return <div style={{ display: 'grid', gap: 12 }}>
    ...
    {state === 'connected' && host && <div style={{ display: 'grid', gap: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
        ...
        <button className="outline" onClick={() => void (screenShare ? stopScreenShare() : startOrChangeScreenShare())} disabled={!screenShareAvailable || screenShareChanging} style={{ ...controlStyle, borderColor: screenShare ? 'rgba(59,130,246,.55)' : 'rgba(255,255,255,.12)' }} title={screenShare ? 'Arrêter le partage d’écran' : 'Partager mon écran'}>
