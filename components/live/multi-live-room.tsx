...
import { Camera, CameraOff, Mic, MicOff, MonitorUp, MonitorStop, Square, Volume2 } from 'lucide-react'
import { Room, RoomEvent, Track } from 'livekit-client'

type Props = { tokenUrl: string; tokenBody: Record<string, string>; host?: boolean; label?: string; onEndLive?: () => void; endingLive?: boolean }
...
function VideoTile({ item, host }: { item: VideoItem; host: boolean }) {
  ...
  useEffect(() => {
    ...
    return () => { try { item.track.detach(video) } catch {}; video.remove() }
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', minWidth: 0, minHeight: 0, overflow: 'hidden', borderRadius: 12, background: '#171922', border: item.local ? '2px solid rgba(255,255,255,.9)' : '1px solid rgba(255,255,255,.35)', boxShadow: '0 8px 24px rgba(0,0,0,.28)' }}>
      <div style={{ position: 'absolute', left: 8, bottom: 8, zIndex: 3, padding: '5px 8px', borderRadius: 7, background: 'rgba(0,0,0,.68)', color: '#fff', fontSize: 11, fontWeight: 800 }}>{item.label}</div>
    </div>
  )
}
...
export default function MultiLiveRoom({ tokenUrl, tokenBody, host = false, label, onEndLive, endingLive = false }: Props) {
  ...
  useEffect(() => {
    ...
    const attachAudio = (track: any) => {
      ...
      el.autoplay = true; el.muted = false; el.volume = 1; el.style.display = 'none'; document.body.appendChild(el)
      ...
      try {
        ...
        if (host) {
          ...
          try { await room.localParticipant.setCameraEnabled(true); if (!cancelled) setCamera(true) } catch (err) {
            if (!cancelled) {
              const name = err instanceof Error ? err.name : ''; const msg = err instanceof Error ? err.message : ''
  ...
  const sortedItems = useMemo(() => [...items.filter((item) => item.local), ...items.filter((item) => !item.local)], [items])
  ...
  async function toggleCamera() {
    const room = roomRef.current; if (!room || !host) return
    try { const next = !camera; await room.localParticipant.setCameraEnabled(next); setCamera(next); if (next) setMessage('Caméra activée.') } catch (err) { setMessage(err instanceof Error ? err.message : "Impossible d'activer la caméra.") }
  }
  ...
  return (
    <div style={{ display: 'grid', gap: 10 }}>
      {host && status === 'connected' && <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', padding: '8px 10px', borderRadius: 12, border: '1px solid var(--line)', background: 'var(--panel)' }}>
        <span className="muted" style={{ fontSize: 12, fontWeight: 800 }}>Arrière-plan</span>
        {Object.entries(BACKGROUNDS).map(([id, item]) => <button key={id} type="button" title={item.label} aria-label={item.label} onClick={() => setBackgroundId(id)} style={{ width: 28, height: 28, borderRadius: 8, border: backgroundId === id ? '2px solid var(--text)' : '1px solid var(--line)', background: item.value, cursor: 'pointer', padding: 0 }} />)}
      </div>}
      ...
      <div style={{ position: 'relative', width: '100%', aspectRatio: '16/9', overflow: 'hidden', borderRadius: 14, background: stageBackground, border: '1px solid var(--line)' }}>
        {!screenTrack && sortedItems.length === 0 && <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', color: '#fff', opacity: 0.8, zIndex: 2, textAlign: 'center', padding: 20 }}>{status === 'connected' ? 'En attente des caméras…' : message}</div>}
        {screenTrack && <div ref={screenRef} style={{ position: 'absolute', inset: 0, zIndex: 1, background: '#000', padding: 4, overflow: 'hidden' }} />}
        {sortedItems.length > 0 && <div style={{ position: 'absolute', inset: screenTrack ? 'auto 8px 8px 8px' : 8, zIndex: 5, display: 'grid', gridTemplateColumns: sortedItems.length === 1 ? 'minmax(0,1fr)' : 'repeat(2,minmax(0,1fr))', gap: 8, height: screenTrack ? 'clamp(96px,22%,180px)' : 'calc(100% - 16px)' }}>
          {sortedItems.slice(0, 4).map((item) => <VideoTile key={`${item.id}-${item.track?.sid || ''}`} item={item} host={host} />)}
        </div>}
      </div>
      {host && status === 'connected' && <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap', alignItems: 'center' }}>
        <button className="outline" onClick={() => void toggleMic()} style={{ minHeight: 36, padding: '0 9px', display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11 }}>{mic ? <Mic size={15} /> : <MicOff size={15} />} {mic ? 'Couper le micro' : 'Activer le micro'}</button>
        <button className="outline" onClick={() => void toggleCamera()} style={{ minHeight: 36, padding: '0 9px', display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11 }}>{camera ? <Camera size={15} /> : <CameraOff size={15} />} {camera ? 'Couper la caméra' : 'Activer la caméra'}</button>
        <button className="outline" onClick={() => void toggleScreen()} style={{ minHeight: 36, padding: '0 9px', display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11 }}>{screenTrack ? <MonitorStop size={15} /> : <MonitorUp size={15} />} {screenTrack ? 'Arrêter le partage' : 'Partager mon écran'}</button>
        {onEndLive && <button type="button" onClick={onEndLive} disabled={endingLive} style={{ minHeight: 36, padding: '0 9px', display: 'inline-flex', alignItems: 'center', gap: 6, border: '1px solid rgba(239,68,68,.5)', background: 'rgba(127,29,29,.16)', color: '#f87171', fontWeight: 800, cursor: endingLive ? 'wait' : 'pointer', borderRadius: 8, fontSize: 11 }}><Square size={14} fill="currentColor" /> {endingLive ? 'Arrêt…' : 'Couper le Live'}</button>}
      </div>}
      {!host && audioBlocked && status === 'connected' && <button onClick={() => void roomRef.current?.startAudio().then(() => setAudioBlocked(false)).catch(() => {})} style={{ minHeight: 40, border: 0, borderRadius: 10, fontWeight: 800, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><Volume2 size={17} /> Activer le son</button>}
    </div>
  )
}
