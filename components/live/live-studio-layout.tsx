...
import { Camera, LayoutDashboard, MonitorUp, Move, Palette } from 'lucide-react'
...
const BACKGROUNDS: Background[] = [
  ...
  { id: 'cyan', label: 'Bleu cyan', value: 'linear-gradient(135deg,#062a35 0%,#087f9b 55%,#54e0ff 100%)' },
...
export default function LiveStudioLayout({ children, host = false }: { children: ReactNode; host?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!stage) return
    stage.dataset.conikStudioStage = 'true'
    ...
    const applyLayout = () => {
      ...
      if (active && primary === 'screen') {
        ...
        camera.style.boxShadow = '0 14px 35px rgba(0,0,0,.35)'
        camera.style.border = '2px solid rgba(255,255,255,.72)'
  ...
  useEffect(() => {
    ...
    const getPosition = () => {
      ...
      const x = Math.max(8, Math.min(rect.width - c.width - 8, c.left - rect.left))
      const y = Math.max(8, Math.min(rect.height - c.height - 8, c.top - rect.top))
      return { x, y }
    }
    const onPointerDown = (event: PointerEvent) => {
      ...
      startX = event.clientX; startY = event.clientY; originX = pos.x; originY = pos.y
    ...
    const onPointerUp = (event: PointerEvent) => {
      if (activePointer !== event.pointerId) return
      activePointer = null; setDragging(false)
    }
    camera.addEventListener('pointerdown', onPointerDown)
    camera.addEventListener('pointermove', onPointerMove)
    camera.addEventListener('pointerup', onPointerUp)
    camera.addEventListener('pointercancel', onPointerUp)
  ...
  return <div ref={rootRef} style={{ display: 'grid', gap: 12 }}>
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 14, border: '1px solid var(--line)', background: 'var(--panel)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 800 }}><LayoutDashboard size={17}/> Mise en page du Live</div>
      <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
        <button type="button" className={primary === 'camera' ? 'primary' : 'outline'} onClick={() => setPrimary('camera')} style={{ minHeight: 40, display: 'inline-flex', alignItems: 'center', gap: 7 }}><Camera size={15}/> Caméra en avant</button>
        {hasScreen && <button type="button" className={primary === 'screen' ? 'primary' : 'outline'} onClick={() => setPrimary('screen')} style={{ minHeight: 40, display: 'inline-flex', alignItems: 'center', gap: 7 }}><MonitorUp size={15}/> Écran en avant</button>}
      </div>
    </div>

    {!hasScreen && <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', padding: '10px 12px', borderRadius: 14, border: '1px solid var(--line)', background: 'var(--panel)' }}>
      <Palette size={17}/><b style={{ fontSize: 13 }}>Fond du studio</b>
      <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
        {BACKGROUNDS.map(item => <button key={item.id} type="button" aria-label={`Fond ${item.label}`} title={`Fond ${item.label}`} onClick={() => setBackground(item.id)} style={{ width: 42, height: 30, borderRadius: 9, border: background === item.id ? '2px solid #fff' : '1px solid rgba(255,255,255,.25)', background: item.value, cursor: 'pointer', boxShadow: background === item.id ? '0 0 0 2px #0b5cff' : 'none' }}/>) }
      </div>
      <span className="muted" style={{ fontSize: 12 }}>{BACKGROUNDS.find(item => item.id === background)?.label} · Filigrane Conik.io</span>
    </div>}

    <div style={{ position: 'relative' }}><div style={{ position: 'relative', zIndex: 1 }}>{children}</div></div>
    <div className="muted" style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12 }}><Move size={14}/> Glissez la caméra pour la placer où vous voulez. {hasScreen ? 'Vous pouvez la déplacer sur l’écran partagé.' : 'Elle reste librement positionnable sur le fond.'}</div>
  </div>
}
