...
function validExternalScript(value: string) {
  try {
...
export function FunnelRuntime({
  ...
  if (!page) {
    return (
      <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center' }}>Chargement…</main>
    )
  }
  ...
  const runtimeScripts: RuntimeScript[] = Array.isArray(page.metadata?.runtime_scripts)
    ? page.metadata.runtime_scripts
        ...
        .map((value) => ({
          src:
            typeof value.src === 'string' && validExternalScript(value.src)
              ? value.src
