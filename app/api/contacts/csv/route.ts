...
export async function GET() {
  ...
  for (const c of data || []) lines.push([c.first_name,c.last_name,c.email,c.phone,c.status,c.consent_status === 'opted_in',c.custom_fields?.source || 'manual',c.created_at].map(csvEscape).join(','))
...
export async function POST(request: Request) {
  ...
  if (file.size > 5 * 1024 * 1024) return NextResponse.json({ error: 'Le fichier CSV ne doit pas dépasser 5 Mo.' }, { status: 413 })
  const text = await file.text()
  const rows = parseCsv(text)
  if (rows.length < 2) return NextResponse.json({ error: 'Le CSV doit contenir une ligne d’en-tête et au moins un contact.' }, { status: 400 })
  ...
  if (emailIndex < 0 && phoneIndex < 0) return NextResponse.json({ error: 'Le CSV doit contenir une colonne e-mail ou téléphone.' }, { status: 400 })
  ...
  rows.slice(1).forEach((row, offset) => {
    ...
    if (!email && !phone) { skipped.push({row:line,reason:'e-mail et téléphone manquants'}); return }
    if (email && !emailValid(email)) { skipped.push({row:line,reason:'e-mail invalide'}); return }
    if ((email && (emails.has(email) || seenEmails.has(email))) || (phone && (phones.has(phone) || seenPhones.has(phone)))) { skipped.push({row:line,reason:'e-mail ou téléphone en doublon'}); return }
    const consentRaw = consentIndex >= 0 ? String(row[consentIndex] || '').trim().toLowerCase() : ''
    const consent = ['true','yes','1','oui','granted','opted_in'].includes(consentRaw)
    toInsert.push({organization_id:membership.organizationId,email,phone,first_name:firstIndex>=0?clean(row[firstIndex],120):null,last_name:lastIndex>=0?clean(row[lastIndex],120):null,status:'lead',consent_status:consent?'opted_in':'unknown',custom_fields:{source:sourceIndex>=0?clean(row[sourceIndex],160)||'import_csv':'import_csv'}})
