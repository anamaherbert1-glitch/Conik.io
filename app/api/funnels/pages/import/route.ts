...
/** Extract inline <script> bodies in document order; remove them from HTML. */
function extractInlineScripts(html: string) {
  ...
  const clean = html.replace(/<script\b([^>]*)>([\s\S]*?)<\/script>/gi, (whole, attrs: string, body: string) => {
    // Keep external src scripts out of the inline bucket (handled separately).
...
/** Absolute http(s) script URLs, document order, max 20. */
function extractExternalScripts(html: string) {
  ...
  while ((match = re.exec(html)) !== null) {
    ...
    try {
      ...
      // relative scripts are rewritten via assets when coming from a ZIP
    }
  }
  ...
  try {
    ...
    if (!parsed.success || !(file instanceof File)) {
      return NextResponse.json(
        { error: 'La page et le fichier HTML/ZIP sont obligatoires.' },
        { status: 400 },
      )
    }
    if (file.size <= 0 || file.size > MAX_UPLOAD) {
      return NextResponse.json(
        { error: 'Le fichier doit peser entre 1 octet et 25 Mo.' },
        { status: 413 },
      )
    }
    ...
    if (isZip) {
      ...
      const assets = entries
        .filter((e) => e.name !== htmlEntry.name && assetMime(e.name))
        .slice(0, MAX_ASSETS)
      ...
      for (const entry of assets) {
        ...
        if (assetError) {
          throw new Error(`Enregistrement de « ${entry.name} » impossible : ${assetError.message}`)
        }
        assetUrls.set(
          entry.name,
      ...
      // Preserve onclick / on* / audio / video / data-* — only strip <script> and <base>
      ...
      const withStyles = extractStyles(raw, htmlEntry.name, (cssPath) => {
        ...
        return body === undefined
          ? null
          : rewriteCssUrls(body, cssPath, (p) => assetUrls.get(p) || null)
      })
      rawHtml = rewriteHtmlRefs(
        extractBody(withStyles.html),
        htmlEntry.name,
        (p) => assetUrls.get(p) || null,
        () => `/${funnel.slug}/${page.slug}`,
      )
      ...
      if (!/\.html?$/i.test(file.name) && file.type !== 'text/html') {
        return NextResponse.json(
          { error: 'Envoyez un fichier .html ou un fichier .zip contenant un HTML.' },
          { status: 400 },
        )
      }
    ...
    return NextResponse.json(
      { error: e?.message || 'Import de la page impossible.' },
      { status: 400 },
    )
  }
}
