...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isZip) {
      ...
      const raw = sanitizeImportedHtml(textFrom(htmlEntry.data, MAX_HTML_BYTES))
      ...
      rawHtml = extractBody(sanitizeImportedHtml(textFrom(Buffer.from(await file.arrayBuffer()), MAX_HTML_BYTES)));
    }
    ...
    const { data: version, error: versionError } = await supabase.from('funnel_versions').insert({ page_id: page.id, version_number: next, html: rawHtml, css, js: '', metadata: { imported: true, scriptsRemoved: true, source: sourceName, assets: assetUrls.size } }).select('id,version_number,html,css,js,metadata').single()
