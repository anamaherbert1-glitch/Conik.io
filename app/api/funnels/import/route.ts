...
function extractForms(html: string) {
  ...
  for (const match of formMatches) {
    ...
    const controls = body.matchAll(/<(input|textarea|select)\b([^>]*)>/gi)
    let position = 0
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const baseName = requestedName || file.name.replace(/\.zip$/i, '').replace(/[_-]+/g, ' ')
    ...
    const { data: funnel, error: funnelError } = await supabase.from('funnels').insert({ organization_id: membership.organizationId, name: baseName.slice(0, 160), slug, source: 'import', created_by: user.id, status: 'draft' }).select('id').single()
    ...
    const assetsByOriginal = new Map((assetInsert.data || []).map((a) => [a.original_name, a.id]))
    const ordered = [...htmlEntries].sort((a, b) => {
      const aHome = /(^|\/)index\.html?$/i.test(a.name) ? 0 : 1
      const bHome = /(^|\/)index\.html?$/i.test(b.name) ? 0 : 1
    ...
    for (let index = 0; index < ordered.length; index++) {
      ...
      let html = sanitizeHtml(textFrom(entry.data))
      ...
      for (const script of html.matchAll(/<script\b([^>]*)>([\s\S]*?)<\/script>/gi)) if (script[2].trim()) jsParts.push(script[2].trim())
      const versionMetadata = { imported_file: entry.name, detected_forms: forms.length, forms, detected_ctas: ctas.length, ctas, asset_count: assets.length, security: { scripts_stored_but_not_executed: true, dangerous_markup_removed: true } }
      const { data: version, error: versionError } = await supabase.from('funnel_versions').insert({ page_id: page.id, version_number: 1, html, css: '', js: jsParts.join('\n\n'), metadata: versionMetadata, created_by: user.id }).select('id').single()
      if (versionError || !version) throw new Error(`Unable to create version for ${entry.name}: ${versionError?.message || 'unknown error'}`)
      await supabase.from('funnel_assets').update({ version_id: version.id }).eq('funnel_id', funnel.id)
      pages.push({ id: page.id, slug: pageSlug, name: rawName })
    }

    if (assetRows.length) {
      const linkedAssets = assetInsert.data || []
      for (const asset of linkedAssets) {
        const sourceEntry = byName.get(asset.original_name.toLowerCase())
        if (!sourceEntry) continue
      }
    }

    return NextResponse.json({ ok: true, funnel: { id: funnel.id, name: baseName, slug }, pages, analysis: { html_pages: htmlEntries.length, assets: assets.length, forms: pages.length, total_files: entries.length, security: 'sanitized_and_sandbox_ready' } })
  } catch (error) {
