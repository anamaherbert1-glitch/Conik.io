...
export async function GET(request: NextRequest) {
  try {
    ...
    for(const row of safeConversions){const currency=String(row.currency||'').trim().toUpperCase()||'UNKNOWN';revenueByCurrency[currency]=(revenueByCurrency[currency]||0)+Number(row.amount||0)}
    const currencyKeys=Object.keys(revenueByCurrency)
    const revenue=currencyKeys.length===1&&currencyKeys[0]!=='UNKNOWN'?revenueByCurrency[currencyKeys[0]]:null
    const by:Record<string,{date:string;views:number;clicks:number;submissions:number;conversions:number}>={}
    for(let i=days-1;i>=0;i--){const d=new Date(Date.now()-i*86400000).toISOString().slice(0,10);by[d]={date:d,views:0,clicks:0,submissions:0,conversions:0}}
    safeViews.forEach((x:any)=>{const d=x.created_at.slice(0,10);if(by[d])by[d].views++}); safeClicks.forEach((x:any)=>{const d=x.created_at.slice(0,10);if(by[d])by[d].clicks++}); safeSubs.forEach((x:any)=>{const d=x.created_at.slice(0,10);if(by[d])by[d].submissions++}); safeConversions.forEach((x:any)=>{const d=x.created_at.slice(0,10);if(by[d])by[d].conversions++})
    return NextResponse.json({pageViews:safeViews.length,visitors,clicks:safeClicks.length,submissions:safeSubs.length,leads:safeSubs.length,conversions:safeConversions.length,revenue,revenueByCurrency,byDay:Object.values(by)})
  } catch (e: any) { return NextResponse.json({ error: e?.message || 'Non autorisé' }, { status: 401 }) }
}
