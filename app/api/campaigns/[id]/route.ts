...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){const {supabase,organization}=await requireWorkspaceRole(['owner','admin','editor']);const{id}=await params;const parsed=schema.safeParse(await request.json().catch(()=>null));if(!parsed.success)return NextResponse.json({error:'Invalid campaign data.'},{status:400});if(parsed.data.funnel_id){const{data:f}=await supabase.from('funnels').select('id').eq('id',parsed.data.funnel_id).eq('organization_id',organization.id).maybeSingle();if(!f)return NextResponse.json({error:'Invalid funnel.'},{status:400})}const{data,error}=await supabase.from('campaigns').update(parsed.data).eq('id',id).eq('organization_id',organization.id).select('id,name,status,funnel_id,created_at,updated_at').single();if(error)return NextResponse.json({error:error.message},{status:400});return NextResponse.json({campaign:data})}
