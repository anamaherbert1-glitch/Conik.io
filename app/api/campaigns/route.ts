...
export async function POST(request: Request) {
  ...
  if (parsed.data.funnel_id) { const {data:f}=await supabase.from('funnels').select('id').eq('id',parsed.data.funnel_id).eq('organization_id',organization.id).maybeSingle(); if(!f)return NextResponse.json({error:'Ce tunnel n’appartient pas à cet espace de travail.'},{status:400}) }
