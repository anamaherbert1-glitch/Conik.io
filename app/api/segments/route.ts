...
export async function POST(request:Request){
  ...
  if(!body||typeof body!=='object')return NextResponse.json({error:'Invalid segment.'},{status:400})
  const name=text(body.name,120);if(!name)return NextResponse.json({error:'Segment name is required.'},{status:400})
...
export async function PATCH(request:Request){
  ...
  const body=await request.json().catch(()=>null);if(!body||typeof body!=='object'||typeof body.id!=='string')return NextResponse.json({error:'Segment id is required.'},{status:400})
  const rules=validRules(body.rules);const name=text(body.name,120);if(!name)return NextResponse.json({error:'Segment name is required.'},{status:400})
  ...
  if(error)return NextResponse.json({error:error.message},{status:400});if(!data)return NextResponse.json({error:'Segment not found.'},{status:404})
  return NextResponse.json({segment:data})
}

export async function DELETE(request:Request){
  const {supabase,membership}=await requireWorkspaceRole(['owner','admin'])
  const body=await request.json().catch(()=>null);if(!body||typeof body.id!=='string')return NextResponse.json({error:'Segment id is required.'},{status:400})
