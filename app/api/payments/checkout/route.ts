...
export async function GET(request: Request) {
  ...
  if ((!funnelSlug && !pageSlug) || !tariffSlug) {
    return NextResponse.json({ error: 'Paramètres page/funnel et tarif requis' }, { status: 400 })
  }
  ...
  try {
    ...
    if (pageSlug) {
      const { data, error } = await supabase
        .from('payment_pages')
        .select('id,organization_id,funnel_id,name,slug,status,currency,amount_type,fixed_amount,checkout_config,success_url,cancel_url')
        .eq('slug', pageSlug)
        .eq('status', 'published')
        .maybeSingle()
      ...
      const { data: f } = await supabase
        ...
        .eq('id', paymentPage.funnel_id)
        .eq('organization_id', paymentPage.organization_id)
        .maybeSingle()
      funnel = f
    } else {
      const { data: f } = await supabase
    ...
    if (!funnel || funnel.status !== 'published' || !funnel.payment_enabled) {
      return NextResponse.json({ error: 'Tunnel introuvable, non publié ou paiement désactivé' }, { status: 404 })
    }

    const { data: tariff } = await supabase
      .from('payment_tariffs')
      .select('id,name,amount_cents,currency,slug,product_label,active,funnel_id,organization_id')
    ...
    let provider: { id: string; provider: string; label: string; status: string } | null = null
    ...
    if (paymentPage?.id) {
      const { data } = await supabase
        ...
        .select('id,method_code,display_name,enabled,sort_order,config')
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: tariff } = await supabase
      .from('payment_tariffs')
      .select('id,funnel_id,organization_id,amount_cents,currency,name,product_label,active')
      .eq('id', body.tariffId)
      .maybeSingle()
    ...
    const { data: funnel } = await supabase
      .from('funnels')
      .select('id,organization_id,payment_provider_id,payment_enabled')
      .eq('id', tariff.funnel_id)
      .eq('organization_id', tariff.organization_id)
      .maybeSingle()
    ...
    if (paymentPageId) {
      const { data: page } = await supabase
        .from('payment_pages')
        .select('id,funnel_id,organization_id,status')
        .eq('id', paymentPageId)
        .eq('funnel_id', funnel.id)
        .eq('organization_id', funnel.organization_id)
        .maybeSingle()
    ...
    const { data: order, error: orderError } = await supabase
      ...
      .insert({
        ...
        subtotal: tariff.amount_cents,
        total_amount: tariff.amount_cents,
        status: 'pending',
        ...
        customer_first_name: body.firstName ? String(body.firstName).slice(0, 80) : null,
        customer_last_name: body.lastName ? String(body.lastName).slice(0, 80) : null,
        product_label: tariff.product_label || tariff.name,
        metadata: { checkout_source: 'conik_checkout' },
      })
      .select('id,order_number,amount_cents,total_amount,currency,status,payment_page_id')
      .single()
    ...
    const { data: transaction, error: transactionError } = await supabase
      ...
      .select('id,order_id,provider_id,amount_cents,currency,status,provider_ref,created_at')
      .single()
    ...
    return NextResponse.json({ order, transaction, message: 'Commande créée. Le prestataire peut maintenant être appelé côté serveur.' }, { status: 201 })
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : 'Erreur serveur paiement' }, { status: 500 })
  }
}
