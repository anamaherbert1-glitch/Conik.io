...
export async function GET(request: Request) {
  ...
  let supabase
  try { supabase = createAdminClient() } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : 'Scheduler configuration missing.' }, { status: 500 }) }
  ...
  for (const action of actions || []) {
    try { await sendAutomationAction(supabase, action.id); completed++ } catch { failed++ }
  }
