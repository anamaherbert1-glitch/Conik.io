...
import { debugToken, exchangeEmbeddedSignupCode, encryptAccessToken, getWaba, listPhoneNumbers, listOwnedWabas, getMetaConfig } from '@/lib/whatsapp/meta'
  ...
  try {
    ...
    if (!wabaId) {
      ...
      if (business) {
        ...
        wabaId = clean(first?.id, 100)
      } else {
        const firstBusiness = businesses.data?.[0]
        business = clean(firstBusiness?.id, 100)
        if (business) {
          const owned = await listOwnedWabas(token.access_token, business)
          const firstWaba = owned.data?.[0]
          wabaId = clean(firstWaba?.id, 100)
        }
      }
    }

    if (!wabaId) {
      return NextResponse.json({ error: 'Meta a bien authentifié le compte, mais aucun WABA n’a pu être identifié. Relancez Embedded Signup et conservez les identifiants WABA/numéro retournés par Meta.', debug: debugData }, { status: 422 })
    }
    ...
    const { data: connectionId, error } = await supabase.rpc('whatsapp_upsert_connection', {
      ...
      p_metadata: {
        metaWabaName: clean(waba.name, 200),
        tokenType: token.token_type || 'bearer',
        debugAppId: debugData.app_id || null,
        connectedVia: 'embedded_signup_v4',
      },
    })
