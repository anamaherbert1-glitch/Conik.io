...
export async function POST(request: Request) {
  ...
  const { data: connection } = await supabase.from('whatsapp_connections').select('id,waba_id,status').eq('id', connectionId).eq('organization_id', membership.organizationId).maybeSingle()
  ...
  if (audienceType === 'tag') {
    ...
    const { data: tagged } = await supabase.from('contact_tags').select('contact_id').eq('tag_id', tagId)
