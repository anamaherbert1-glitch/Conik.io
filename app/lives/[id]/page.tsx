...
import { ArrowLeft, Copy, Radio, Trash2, Users, Square, CircleCheck, Clock3, Send, Smartphone, Link2, MessageSquare, MessageSquareOff } from 'lucide-react'
...
export default function LiveDetail({params}:{params:Promise<{id:string}>}){
  ...
  async function endLive(){if(!live||ending)return;setEnding(true);setError('');const r=await fetch(`/api/lives/${live.id}/end`,{method:'POST'});const j=await r.json().catch(()=>({}));if(!r.ok){setError(j.error||'Impossible de terminer le Live.');setEnding(false);return}setLive(v=>v?{...v,status:'ended'}:v);setStudio(false);setEndConfirmOpen(false);setNotice('Live terminé.');setEnding(false)}
  async function toggleChat(){if(!live||chatSaving)return;setChatSaving(true);setError('');const enabled=!live.chat_enabled;const r=await fetch(`/api/lives/${live.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_enabled:enabled})});const j=await r.json().catch(()=>({}));if(!r.ok){setError(j.error||'Impossible de modifier le chat.')}else{setLive(v=>v?v?{...v,chat_enabled:j.chat_enabled}:v:v);setNotice(enabled?'Chat des followers activé.':'Chat des followers désactivé.')}setChatSaving(false)}
  ...
  const inviteSidebar=<div style={{display:'grid',gap:9}}>
    ...
    <div style={{display:'grid',gap:6}}>
      <div className="choice" style={{padding:'7px 8px',fontSize:11}}><div style={{display:'flex',alignItems:'center',gap:6}}><CircleCheck size={13}/><b>Connectés</b><span className="muted">{connected.length}</span></div>{connected.length>0&&<div style={{marginTop:5,display:'grid',gap:4}}>{connected.slice(0,4).map(a=><div key={a.participant_id} style={{fontSize:10,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{a.display_name||a.email}</div>)}</div>}</div>
      <div className="choice" style={{padding:'7px 8px',fontSize:11}}><div style={{display:'flex',alignItems:'center',gap:6}}><Clock3 size={13}/><b>En attente</b><span className="muted">{notConnected.length}</span></div>{notConnected.length>0&&<div style={{marginTop:5,display:'grid',gap:4}}>{notConnected.slice(0,4).map(p=><div key={p.id} style={{fontSize:10,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{contactName(p.contact_id,p.email)}</div>)}</div>}</div>
    </div>
    <div style={{borderTop:'1px solid var(--line)',paddingTop:8}}>
      <div className="muted" style={{fontSize:10,fontWeight:800,marginBottom:5}}>CHAT DES FOLLOWERS</div>
      <button type="button" onClick={()=>void toggleChat()} disabled={chatSaving||live.status==='ended'} className={live.chat_enabled?'outline':'primary'} style={{width:'100%',minHeight:38,display:'flex',alignItems:'center',justifyContent:'center',gap:7,fontSize:11,fontWeight:800}}>{live.chat_enabled?<><MessageSquareOff size={14}/>{chatSaving?'Modification…':'Couper le chat'}</>:<><MessageSquare size={14}/>{chatSaving?'Modification…':'Réactiver le chat'}</>}</button>
      <div className="muted" style={{fontSize:10,textAlign:'center',marginTop:5}}>{live.chat_enabled?'Les followers peuvent envoyer des messages.':'Les followers peuvent lire les messages, mais ne peuvent plus en envoyer.'}</div>
    </div>
    <div style={{borderTop:'1px solid var(--line)',paddingTop:8}}>
      <div className="muted" style={{fontSize:10,fontWeight:800,marginBottom:5}}>AJOUTER UN CONTACT</div>
      <select className="form-input" value={selected} onChange={e=>setSelected(e.target.value)} style={{width:'100%',fontSize:11,minHeight:36}}><option value="">Choisir…</option>{contacts.filter(c=>c.email&&!allowed.has(c.id)).map(c=><option key={c.id} value={c.id}>{[c.first_name,c.last_name].filter(Boolean).join(' ')||c.email}</option>)}</select>
      <button className="primary" onClick={add} disabled={!selected} style={{width:'100%',minHeight:35,marginTop:6,fontSize:11}}>Autoriser</button>
    </div>
    {participants.length>0&&<div style={{borderTop:'1px solid var(--line)',paddingTop:8,display:'grid',gap:5}}>{participants.slice(0,8).map(p=><div key={p.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:6,fontSize:10}}><span style={{minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{contactName(p.contact_id,p.email)}</span><button className="outline" onClick={()=>remove(p.contact_id)} aria-label="Retirer" style={{width:28,height:28,padding:0,display:'grid',placeItems:'center',flex:'0 0 auto'}}><Trash2 size={12}/></button></div>)}</div>}
  </div>
  ...
  return <AppShell active="Live Events"><div className="page" style={{maxWidth:1180}}>
    ...
    {!live?<div className="emptybox big">Chargement…</div>:<>
      ...
      {studio&&live.status!=='ended'&&<section className="panel" style={{marginBottom:18}}><div style={{display:'flex',alignItems:'center',gap:9,marginBottom:14}}><Radio size={18}/><h3 style={{margin:0}}>Studio</h3></div><LiveStudioLayout host sidebarContent={inviteSidebar}><LiveRoom tokenUrl={`/api/lives/${live.id}/token`} tokenBody={{}} host/></LiveStudioLayout></section>}
      ...
      {live.status!=='ended'&&<section style={{marginBottom:18}}><LiveChat liveId={live.id} host chatEnabled={live.chat_enabled}/></section>}
      <section className="panel" style={{marginTop:18}}>
        <div style={{display:'flex',alignItems:'center',gap:9,marginBottom:5}}><Smartphone size={18}/><h3 style={{margin:0}}>WhatsApp</h3><span className="muted" style={{fontSize:12,marginLeft:'auto'}}>{waEligible}/{waInvited} éligibles</span></div>
