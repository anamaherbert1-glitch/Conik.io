...
function detectButtons(html:string):ButtonItem[]{
  ...
  return Array.from(doc.querySelectorAll('a,button,input[type="button"],input[type="submit"]')).map((el,index)=>({index,label:(el.textContent||el.getAttribute('value')||'').trim().replace(/\s+/g,' ')||`Bouton ${index+1}`,href:el.getAttribute('href')||''}))
}
