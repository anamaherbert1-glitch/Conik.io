...
function applyButtonRedirects(source: string, controls: RedirectControl[]) {
  ...
  let html = source.replace(/<(a|button)\b([^>]*)>([\s\S]*?)<\/\1>/gi, (full, tag, attrs, body) => {
    ...
    if (lower === 'a') {
      const replaced = /\bhref\s*=\s*(["'])[^"']*\1/i.test(attrs) ? attrs.replace(/\bhref\s*=\s*(["'])[^"']*\1/i, `href="${target}"`) : `${attrs} href="${target}"`
  ...
  html = html.replace(/<input\b([^>]*\btype=["'](?:button|submit|reset)["'][^>]*)(\/?\s*)>/gi, (full, attrs, end) => {
    const c = controls[i++]
    if (!c?.target) return full
...
export default function FunnelEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function selectPage(page: Page) {
    ...
    const { data: v, error } = await createClient().from('funnel_versions').select('id,version_number,html,css,js,metadata').eq('page_id', page.id).order('version_number', { ascending: false }).limit(1).maybeSingle()
    if (error) { setMessage(error.message); return }
    setVersion(v)
    if (v) {
      setHtml(v.html); setCss(v.css); setJs(v.js)
      setShareImageUrl(typeof v.metadata?.share_image_url === 'string' ? v.metadata.share_image_url : '')
      ...
      setRedirectControls(detected.map((d, i) => ({ ...d, target: stored[i]?.target || d.target || '', existing: Boolean(stored[i]?.target || d.existing || d.target), actionType: stored[i]?.target ? 'url' : d.actionType })))
    } else { setHtml(DEFAULT_HTML); setCss(DEFAULT_CSS); setJs(''); setShareImageUrl(''); setRedirectControls(detectRedirectControls(DEFAULT_HTML)) }
  }
  ...
  async function createPage() {
    ...
    const { data: v, error: versionError } = await supabase.from('funnel_versions').insert({ page_id: data.id, version_number: 1, html: DEFAULT_HTML, css: DEFAULT_CSS, js: '', metadata: { editor: 'conik', redirects: [] } }).select('id,version_number,html,css,js,metadata').single()
  ...
  async function importHtmlOrZip(file: File) {
    if (!selected) return; setBusy(true); setMessage('Import HTML/ZIP en cours…')
    try { const body = new FormData(); body.append('file', file); body.append('pageId', selected.id); const response = await fetch('/api/funnels/pages/import', { method: 'POST', body }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Import impossible.'); setVersion(data.version); setHtml(data.version.html); setCss(data.version.css); setJs(data.version.js); setName(data.page.name); const detected = detectRedirectControls(data.version.html); setRedirectControls(detected); setSelectedControl(null); setActiveCodeTab('html'); setShowCode(true); setMessage(`HTML/ZIP importé avec succès · ${data.assets} asset(s). ${detected.length} élément(s) interactif(s) détecté(s).`) } catch (err) { setMessage(err instanceof Error ? err.message : 'Import impossible.') } finally { setBusy(false) }
  }
  ...
  async function saveVersion() {
    if (!selected) return; const cleanName = name.trim(), clean = cleanSlug(slug); if (!cleanName || !clean) { setMessage('Le nom et le slug de la page sont obligatoires.'); return }
    if (pages.some((p) => p.id !== selected.id && p.slug === clean)) { setMessage('Cet identifiant de page est déjà utilisé dans ce tunnel.'); return }
    const targets = redirectControls.map((c) => safeRedirect(c.target)); if (redirectControls.some((c, i) => c.target.trim() && !targets[i])) { setMessage('Chaque lien de redirection doit être une URL http(s) ou un chemin interne commençant par /.'); return }
    setBusy(true); setMessage(''); const normalized = redirectControls.map((c, i) => ({ ...c, target: targets[i] })); const finalHtml = applyButtonRedirects(html, normalized); const supabase = createClient()
    ...
    if (error) setMessage(error.message); else { setVersion(v); setHtml(finalHtml); setRedirectControls(detectRedirectControls(finalHtml).map((d, i) => ({ ...d, target: normalized[i]?.target || d.target || '', existing: Boolean(normalized[i]?.target || d.existing), actionType: normalized[i]?.target ? 'url' : d.actionType }))); setSelected((p) => (p ? { ...p, name: cleanName, slug: clean } : p)); setPages((ps) => ps.map((p) => (p.id === selected.id ? { ...p, name: cleanName, slug: clean } : p))); setMessage(`Version ${next} enregistrée.`) }
    setBusy(false)
  }
  ...
  async function uploadShareImage(file: File) {
    if (!selected) return
    setBusy(true)
    setMessage('Import de l’image de partage…')
    try {
      ...
      setVersion((current) =>
        current
          ? { ...current, metadata: { ...(current.metadata || {}), share_image_url: data.imageUrl } }
          : current,
      )
      setMessage('Image de partage enregistrée. Lors du partage du lien, cette photo s’affichera à la place du logo Conik.')
    } catch (err) {
      setMessage(err instanceof Error ? err.message : 'Import de l’image impossible.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return <div className="page">
    ...
    <div className="head"><div><small>ÉDITEUR DE TUNNEL</small><h1>{funnel.name}</h1><p>/{funnel.slug} · {funnel.status}</p></div><div className="button-row">{funnel.status === 'published' && <a className="outline" href={`/${funnel.slug}`} target="_blank" rel="noreferrer"><Globe2 size={15} />Ouvrir le tunnel</a>}<button className="primary" onClick={saveVersion} disabled={busy || !selected}><Save size={15} />{busy ? 'Enregistrement…' : 'Enregistrer la version'}</button><button className="primary" onClick={publish} disabled={busy || !version}><Globe2 size={15} />Publier</button></div></div>
    {message && <div className="notice">{message}</div>}
    <div className="page-tabs"><div className="page-tabs-scroll">{pages.map((p) => { const isSelected = selected?.id === p.id; return <div key={p.id} className={`page-tab-wrap ${isSelected ? 'active' : ''}`}><button className="page-tab" onClick={() => void selectPage(p)}><span>Page {p.position + 1}</span><small>/{p.slug}</small></button><button className="page-visibility" onClick={(e) => { e.stopPropagation(); if (isSelected) setShowCode((v) => !v); else void selectPage(p).then(() => setShowCode(false)) }} title="Afficher/masquer le code">{isSelected && showCode ? <EyeOff size={14} /> : <Eye size={14} />}</button></div> })}</div><button className="page-add page-add-right" onClick={createPage} disabled={busy} title="Ajouter une page"><Plus size={18} /></button></div>
    <div className={`editor-grid ${showCode ? '' : 'editor-grid-preview-only'}`}>
      {showCode && <section className="panel code-panel">
        <div className="section-head"><h3>Page : {selected ? `Page ${selected.position + 1}` : '—'}</h3><div className="button-row editor-actions"><label className="outline upload-label import-button"><UploadCloud size={16} />Importer HTML / ZIP<input ref={htmlInputRef} type="file" accept=".html,.htm,.zip,text/html,application/zip" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) void importHtmlOrZip(f); e.currentTarget.value = '' }} /></label><label className="outline upload-label import-button"><FileType2 size={16} />Importer CSS<input ref={cssInputRef} type="file" accept=".css,text/css" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) void importTextFile(f, 'css'); e.currentTarget.value = '' }} /></label><label className="outline upload-label import-button"><FileCode2 size={16} />Importer JavaScript<input ref={jsInputRef} type="file" accept=".js,.mjs,text/javascript,application/javascript" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) void importTextFile(f, 'js'); e.currentTarget.value = '' }} /></label><button className="danger-button page-delete-button" onClick={() => void deletePage()} disabled={busy || !selected}><Trash2 size={15} />Supprimer</button></div></div>
        <div className="field" style={{ marginBottom: 12 }}><span>Nom de la page</span><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Page de capture" /></div>
        <div className="field" style={{ marginBottom: 12 }}><span>Identifiant (slug)</span><input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="page-de-capture" /></div>
        {selected && <div className="panel" style={{ marginBottom: 16, padding: '12px 14px', background: 'var(--code-bg)', border: '1px solid var(--line)' }}>
          <div className="section-head" style={{ marginBottom: 8 }}>
            <div>
              <h4 style={{ margin: 0 }}>Lien public de cette page</h4>
              <span className="muted" style={{ fontSize: 12 }}>Copiez ce lien pour le coller dans un bouton d’une autre page ou le partager. L’image ci-dessous s’affichera en aperçu (WhatsApp, Facebook, etc.) à la place du logo Conik.</span>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
            <code style={{ flex: 1, minWidth: 200, fontSize: 13, wordBreak: 'break-all', padding: '8px 10px', background: 'var(--panel)', borderRadius: 8, border: '1px solid var(--line)' }}>{pagePublicUrl(selected.slug)}</code>
            <button type="button" className="outline" onClick={() => void copyPageLink()}>{copied ? <Check size={15} /> : <Copy size={15} />}{copied ? 'Copié !' : 'Copier'}</button>
            <a className="outline" href={pagePublicPath(selected.slug)} target="_blank" rel="noreferrer"><Globe2 size={15} />Ouvrir</a>
          </div>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ width: 120, height: 72, borderRadius: 10, overflow: 'hidden', border: '1px solid var(--line)', background: 'var(--panel)', display: 'grid', placeItems: 'center', flex: '0 0 auto' }}>
              {shareImageUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={shareImageUrl} alt="Aperçu partage" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <span className="muted" style={{ fontSize: 11, textAlign: 'center', padding: 6 }}>Pas d’image</span>
              )}
            </div>
            <div style={{ flex: 1, minWidth: 200 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 4 }}>Photo de partage de cette page</div>
              <div className="muted" style={{ fontSize: 12, marginBottom: 8 }}>JPG, PNG ou WebP · max 8 Mo · format recommandé 1200×630</div>
              <label className="outline upload-label import-button" style={{ display: 'inline-flex', cursor: busy ? 'not-allowed' : 'pointer' }}>
                <ImageIcon size={15} />
                {shareImageUrl ? 'Changer la photo' : 'Importer une photo'}
                <input ref={shareImageInputRef} type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" hidden disabled={busy} onChange={(e) => { const f = e.target.files?.[0]; if (f) void uploadShareImage(f); e.currentTarget.value = '' }} />
              </label>
            </div>
          </div>
        </div>}
        <div className="redirect-fields"><div className="section-head"><div><h4>Éléments interactifs détectés</h4><span className="muted">Les éléments déjà configurés sont conservés. Les éléments sans destination peuvent recevoir une redirection.</span></div><span className="muted">{redirectControls.length} détecté(s)</span></div>
          {redirectControls.length ? redirectControls.map((c, i) => { const internalSlug = selectedInternalSlug(c.target); const custom = internalSlug === '__custom__'; const isActive = selectedControl === i; return <div key={c.key} className="field redirect-field" style={{ marginBottom: 10, border: isActive ? '1px solid var(--accent)' : '1px solid var(--line)', borderRadius: 10, padding: 10 }} onClick={() => setSelectedControl(i)}><span style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}><span>{c.label} <small>({c.tag})</small></span><strong style={{ fontSize: 11 }}>{c.existing ? 'Déjà configuré' : 'À configurer'}</strong></span>{c.existing && c.actionType === 'onclick' && !c.target ? <span className="muted" style={{ fontSize: 12 }}>Action JavaScript existante — conservée.</span> : <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}><select value={internalSlug} onChange={(e) => { const val = e.target.value; if (val === '__custom__') { if (c.target && internalSlug !== '__custom__') setRedirectTarget(i, '') } else onSelectInternalPage(i, val) }}><option value="">— Aucune redirection —</option>{pages.filter((p) => p.id !== selected?.id).map((p) => <option key={p.id} value={p.slug}>Page {p.position + 1} — {p.name} (/{p.slug})</option>)}<option value="__custom__">URL personnalisée / externe…</option></select>{custom && <input value={c.target} onChange={(e) => setRedirectTarget(i, e.target.value)} placeholder="https://exemple.com ou /mon-chemin" />}{!custom && c.target && <span className="muted" style={{ fontSize: 12 }}>Destination : {c.target}</span>}</div>}</div> }) : <div className="muted">Aucun élément interactif détecté dans cette page.</div>}
        </div>
        <div className="code-tabs"><button className={activeCodeTab === 'html' ? 'active' : ''} onClick={() => setActiveCodeTab('html')}>HTML</button><button className={activeCodeTab === 'css' ? 'active' : ''} onClick={() => setActiveCodeTab('css')}>CSS</button><button className={activeCodeTab === 'js' ? 'active' : ''} onClick={() => setActiveCodeTab('js')}>JavaScript</button></div>
      ...
      <section className="panel preview-panel"><div className="section-head"><h3>Aperçu</h3><span className="muted">Page {selected ? selected.position + 1 : '—'}</span></div><iframe title="Aperçu de la page" sandbox="allow-scripts allow-forms" srcDoc={preview} style={{ width: '100%', minHeight: 650, border: '1px solid #e5e7eb', borderRadius: 12, background: '#fff' }} /></section>
    </div>
  </div>
}
