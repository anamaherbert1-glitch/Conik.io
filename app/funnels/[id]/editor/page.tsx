...
export default function FunnelEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function importHtmlOrZip(file: File) {
    ...
    try {
      ...
      setMessage(`Import terminé · ${data.assets ?? 0} asset(s). Cliquez sur Publier.`)
    } catch (err) {
  ...
  async function importTextFile(file: File, kind: 'css' | 'js') {
    ...
    try {
      ...
      setMessage(`${kind.toUpperCase()} importé. Enregistrez pour conserver.`)
  ...
  async function saveVersion() {
    ...
    try {
      ...
      const total = finalHtml.length + css.length + js.length
      const body: Record<string, unknown> = { pageId: selected.id, redirects: normalized, ...(shareImageUrl ? { shareImageUrl } : {}) }
      if (total > 400000) {
        setMessage('Envoi du contenu volumineux…')
        const supabase = createClient()
        const { data: { user } } = await supabase.auth.getUser()
        ...
        const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://ndsksabyzxfmhnyykcfb.supabase.co'
        const anonKey = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'sb_publishable_-adOy-Xd9Xuqugx74Cjklg_CV9EzTfF'
        const base = `${member.organization_id}/import-temp/${crypto.randomUUID()}`
        async function put(path: string, content: string, mime: string) {
          await new Promise<void>((resolve, reject) => {
            const xhr = new XMLHttpRequest()
            xhr.open('POST', `${supabaseUrl}/storage/v1/object/funnel-assets/${path}`)
            xhr.setRequestHeader('Authorization', `Bearer ${token}`)
            xhr.setRequestHeader('apikey', anonKey)
            xhr.setRequestHeader('x-upsert', 'true')
            xhr.setRequestHeader('Content-Type', mime)
            xhr.onload = () => (xhr.status >= 200 && xhr.status < 300 ? resolve() : reject(new Error(`Envoi échoué (${xhr.status})`)))
            xhr.onerror = () => reject(new Error('Réseau interrompu.'))
            xhr.timeout = 180000
            xhr.send(new Blob([content], { type: mime }))
          })
          return path
        }
        body.htmlPath = await put(`${base}/page.html`, finalHtml, 'text/html;charset=utf-8')
        body.cssPath = await put(`${base}/page.css`, css, 'text/css;charset=utf-8')
        body.jsPath = await put(`${base}/page.js`, js, 'text/javascript;charset=utf-8')
        setMessage('Finalisation de l\'enregistrement…')
      } else {
        body.html = finalHtml
        body.css = css
        body.js = js
      }
      const response = await fetch('/api/funnels/pages/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      const textRes = await response.text()
      let data: any = {}
      try { data = textRes ? JSON.parse(textRes) : {} } catch { throw new Error(textRes.slice(0, 180) || `Erreur serveur (${response.status}).`) }
      if (!response.ok) throw new Error(data.error || 'Enregistrement impossible.')
      setVersion(data.version); setHtml(data.version.html); setCss(data.version.css); setJs(data.version.js)
        ...
        <div className="page-tabs-scroll">
          {pages.map((p) => (
            <div key={p.id} className="page-tab-wrap">
              ...
              <span className="page-visibility" title={p.published_version_id ? 'Publiée' : 'Brouillon'}>{p.published_version_id ? '●' : '○'}</span>
            </div>
          ))}
          <button className="page-add page-add-right" onClick={() => void createPage()} disabled={busy} title="Nouvelle page"><Plus size={16} /></button>
        </div>
      </div>
          ...
          <section className="panel code-panel">
            <div className="section-head">
              ...
              <div className="button-row editor-actions">
                ...
                <label className="outline upload-label import-button"><FileCode2 size={16} />Importer JS
                ...
                <button className="danger-button page-delete-button" onClick={() => void deletePage()} disabled={busy}><Trash2 size={15} />Supprimer</button>
              </div>
            </div>

            <div className="field" style={{ margin: '12px 18px' }}>
              <h4 style={{ margin: '0 0 8px' }}>Lien public</h4>
              <div className="button-row">
              ...
              <div style={{ marginTop: 12 }}>
                <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 4 }}>Photo de partage</div>
                {shareImageUrl ? <div style={{ width: 160, height: 84, borderRadius: 8, overflow: 'hidden', marginBottom: 8 }}><img src={shareImageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div> : null}
                <label className="outline upload-label import-button" style={{ display: 'inline-flex', cursor: busy ? 'not-allowed' : 'pointer' }}>
                  <ImageIcon size={15} />{shareImageUrl ? 'Changer' : 'Importer une photo'}
            ...
            <textarea
              className="code-editor code-input-large"
              value={codeValue}
              onChange={(e) => {
                setCodeValue(e.target.value)
                if (activeCodeTab === 'html') {
                  const detected = detectRedirectControls(e.target.value)
                  setRedirectControls(detected.map((d, i) => ({ ...d, target: redirectControls[i]?.target || d.target || '', existing: Boolean(redirectControls[i]?.target || d.existing || d.target) })))
                }
              }}
              spellCheck={false}
              style={{ background: '#0d1117', color: '#e6edf3', caretColor: '#ea580c', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace', fontSize: 13, lineHeight: 1.55 }}
            />

            <div className="field redirect-field" style={{ margin: '16px 18px' }}>
              ...
              {redirectControls.length ? redirectControls.map((c, i) => {
                ...
                return (
                  <div key={c.key} className="redirect-fields" style={{ marginBottom: 10, padding: 10, border: '1px solid #e5e7eb', borderRadius: 8 }}>
