...
export default function FunnelEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function importHtmlOrZip(file: File) {
    ...
    try {
      ...
      setMessage(`Import terminé · ${data.assets ?? 0} asset(s) · ${detected.length} élément(s) interactif(s). Cliquez sur Publier pour mettre en ligne.`)
    } catch (err) {
  ...
  async function publish() {
    ...
    try {
      const { error } = await createClient().rpc('publish_funnel_page', { target_page: selected.id, target_version: version.id })
      ...
      setMessage('Page publiée avec succès. Ouvrez le tunnel pour vérifier le rendu.')
    } catch (err) {
      ...
      {selected && (
        <section className="panel code-panel">
          ...
          <div className="field" style={{ margin: '12px 18px' }}>
            <h4 style={{ margin: '0 0 8px' }}>Lien public de cette page</h4>
            <div className="button-row">
          ...
          <div className="button-row" style={{ margin: '8px 18px' }}>
