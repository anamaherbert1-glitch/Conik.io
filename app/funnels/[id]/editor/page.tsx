...
export default function FunnelEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [message, setMessage] = useState(''), [busy, setBusy] = useState(false), [showCode, setShowCode] = useState(true), [activeCodeTab, setActiveCodeTab] = useState<CodeTab>('html'), [copied, setCopied] = useState(false)
  const htmlInputRef = useRef<HTMLInputElement>(null), cssInputRef = useRef<HTMLInputElement>(null), jsInputRef = useRef<HTMLInputElement>(null)
  ...
  async function selectPage(page: Page) {
    setSelected(page); setName(page.name); setSlug(page.slug); setMessage(''); setShowCode(true); setActiveCodeTab('html'); setCopied(false); setSelectedControl(null)
    ...
    } else { setHtml(DEFAULT_HTML); setCss(DEFAULT_CSS); setJs(''); setRedirectControls(detectRedirectControls(DEFAULT_HTML)) }
  }
  ...
  async function saveVersion() {
    ...
    const { data: v, error } = await supabase.from('funnel_versions').insert({ page_id: selected.id, version_number: next, html: finalHtml, css, js, metadata: { editor: 'conik', redirects: normalized } }).select('id,version_number,html,css,js,metadata').single()
  ...
  return <div className="page">
    ...
    <div className={`editor-grid ${showCode ? '' : 'editor-grid-preview-only'}`}>
      {showCode && <section className="panel code-panel">
        ...
        {selected && <div className="panel" style={{ marginBottom: 16, padding: '12px 14px', background: 'var(--code-bg)', border: '1px solid var(--line)' }}><div className="section-head" style={{ marginBottom: 8 }}><div><h4 style={{ margin: 0 }}>Lien public de cette page</h4><span className="muted" style={{ fontSize: 12 }}>Copiez ce lien pour le coller dans un bouton d’une autre page ou le partager.</span></div></div><div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}><code style={{ flex: 1, minWidth: 200, fontSize: 13, wordBreak: 'break-all', padding: '8px 10px', background: 'var(--panel)', borderRadius: 8, border: '1px solid var(--line)' }}>{pagePublicUrl(selected.slug)}</code><button type="button" className="outline" onClick={() => void copyPageLink()}>{copied ? <Check size={15} /> : <Copy size={15} />}{copied ? 'Copié !' : 'Copier'}</button><a className="outline" href={pagePublicPath(selected.slug)} target="_blank" rel="noreferrer"><Globe2 size={15} />Ouvrir</a></div></div>}
