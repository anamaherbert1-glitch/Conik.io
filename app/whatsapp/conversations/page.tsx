...
export default function WhatsAppConversationsPage() {
  ...
  return <AppShell active="WhatsApp">
    ...
    <section className="panel" style={{display:'grid',gap:14}}>
      ...
      {items.length ? <div className="funnel-table">{items.map((c:any)=><Link key={c.id} href={`/whatsapp/conversations/${c.id}`} className="funnel-row"><div><b>{[c.contact?.first_name,c.contact?.last_name].filter(Boolean).join(' ') || c.contact?.phone || 'Contact WhatsApp'}</b><span>{c.contact?.whatsapp_number || c.contact?.phone || '—'} · {c.status} · {c.last_message_at || '—'}</span></div><strong>{c.unread_count || 0}</strong></Link>)}</div> : <div className="empty"><b>Aucune conversation</b><span>Les conversations apparaîtront après réception d’événements WhatsApp.</span></div>}
