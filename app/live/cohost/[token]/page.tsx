...
function CohostChat({ token }: { token: string }) {
  ...
  return (
    <section className="panel" style={{ display: 'flex', flexDirection: 'column', minHeight: 560 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0', display: 'grid', gap: 8, alignContent: 'start' }}>
        {messages.map((m) => (
          <div key={m.id} style={{ padding: '8px 10px', borderRadius: 10, border: '1px solid var(--line)' }}>
            <b style={{ fontSize: 12 }}>{m.sender_type === 'host' ? (m.sender_email || 'Organisateur') : 'Participant'}</b>
            <div style={{ marginTop: 3 }}>{m.message}</div>
          </div>
        ))}
      ...
      <form onSubmit={send} style={{ display: 'flex', gap: 8 }}>
        <input className="form-input" value={text} maxLength={1000} onChange={(e) => setText(e.target.value)} placeholder="Écrire un message…" />
        <button className="primary" disabled={sending || !text.trim()}>{sending ? '…' : 'Envoyer'}</button>
      </form>
    </section>
  )
}
...
export default function CoHostLivePage() {
  ...
  return <main style={{ minHeight: '100vh', padding: 16, background: 'var(--background)' }}><div style={{ maxWidth: 1400, margin: '0 auto' }}><div style={{ marginBottom: 12 }}><div style={{ fontWeight: 900 }}>Studio co-organisateur</div><div className="muted" style={{ fontSize: 12 }}>Vous êtes connecté au même Live que l’organisateur principal.</div></div><div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,2fr) minmax(360px,1fr)', gap: 16, alignItems: 'start' }}><MultiLiveRoom host label="Co-organisateur" tokenUrl="/api/lives/cohost-token" tokenBody={{ token: rawToken }} /><CohostChat token={rawToken} /></div></div></main>
}
