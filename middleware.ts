...
export async function middleware(request:NextRequest){
  ...
  const pathname=request.nextUrl.pathname;const host=(request.headers.get('host')||'').split(':')[0].toLowerCase();const segments=pathname.split('/').filter(Boolean)
  if(host&&!PLATFORM_HOSTS.has(host)&&segments.length===1&&/^[a-z0-9]+(?:-[a-z0-9]+)*$/i.test(segments[0])){const{data:verifiedDomain}=await supabase.rpc('is_verified_custom_domain',{target_host:host});if(verifiedDomain===true){const rewriteUrl=request.nextUrl.clone();rewriteUrl.pathname=`/r/${segments[0].toLowerCase()}`;const headers=new Headers(request.headers);headers.set('x-conik-host',host);return addSecurityHeaders(NextResponse.rewrite(rewriteUrl,{request:{headers}}))}}
  const{data}=await supabase.auth.getClaims();const user=data?.claims
  if(!isPublicPath(pathname)&&!user){const redirectUrl=request.nextUrl.clone();redirectUrl.pathname='/login';redirectUrl.searchParams.set('next',`${pathname}${request.nextUrl.search}`);return addSecurityHeaders(NextResponse.redirect(redirectUrl))}
