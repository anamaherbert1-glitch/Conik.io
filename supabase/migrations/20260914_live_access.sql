...
create policy live_access_tokens_org_access on public.live_access_tokens
  for all to authenticated
  using (exists (select 1 from public.live_events le where le.id = live_event_id and public.is_org_member(le.organization_id)))
  with check (exists (select 1 from public.live_events le where le.id = live_event_id and public.is_org_member(le.organization_id)));
...
begin
  ...
  where live_event_id = v_live.id and lower(trim(email)) = lower(trim(p_email))
  limit 1;
  ...
  values(v_live.id, v_participant.id, v_hash, now() + interval '12 hours');
...
create or replace function public.verify_live_access(p_slug text, p_token text)
returns table(live_id uuid, title text, description text, scheduled_at timestamptz, timezone text, status text, stream_provider text, stream_id text)
language sql
security definer
set search_path = public, pg_temp
as $$
  select le.id, le.title, le.description, le.scheduled_at, le.timezone, le.status, le.stream_provider, le.stream_id
  from public.live_events le
  join public.live_access_tokens lat on lat.live_event_id = le.id
  where le.slug = lower(trim(p_slug))
    and lat.token_hash = encode(digest(p_token, 'sha256'), 'hex')
